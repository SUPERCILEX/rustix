...
pub fn set_dumpable_behavior(config: DumpableBehavior) -> io::Result<()> {
    unsafe { prctl_2args(PR_SET_DUMPABLE, ptr::without_provenance_mut(config as usize)) }.map(|_r| ())
}
...
pub fn set_unaligned_access_control(config: UnalignedAccessControl) -> io::Result<()> {
    unsafe { prctl_2args(PR_SET_UNALIGN, ptr::without_provenance_mut(config.bits() as usize)) }.map(|_r| ())
}
...
pub fn set_floating_point_emulation_control(
    ...
    unsafe { prctl_2args(PR_SET_FPEMU, ptr::without_provenance_mut(config.bits() as usize)) }.map(|_r| ())
}
...
pub fn set_time_stamp_counter_readability(
    ...
    unsafe { prctl_2args(PR_SET_TSC, ptr::without_provenance_mut(readability as usize)) }.map(|_r| ())
}
...
pub fn set_machine_check_memory_corruption_kill_policy(
    ...
    let (sub_operation, policy) = if let Some(policy) = policy {
        (PR_MCE_KILL_SET, ptr::without_provenance_mut(policy as usize))
    } else {
        (PR_MCE_KILL_CLEAR, null_mut())
    };

    unsafe { prctl_3args(PR_MCE_KILL, ptr::without_provenance_mut(sub_operation), policy) }.map(|_r| ())
}
...
pub unsafe fn set_virtual_memory_map_address(
    ...
    prctl_3args(PR_SET_MM, ptr::without_provenance_mut(option as usize), address).map(|_r| ())
}
...
pub fn speculative_feature_state(
    ...
    let r = unsafe { prctl_2args(PR_GET_SPECULATION_CTRL, ptr::without_provenance_mut(feature as usize))? }
        as c_uint;
...
pub fn configure_io_flusher_behavior(enable: bool) -> io::Result<()> {
    unsafe { prctl_2args(PR_SET_IO_FLUSHER, ptr::without_provenance_mut(usize::from(enable))) }.map(|_r| ())
}

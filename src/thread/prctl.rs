...
pub fn set_keep_capabilities(enable: bool) -> io::Result<()> {
    unsafe { prctl_2args(PR_SET_KEEPCAPS, ptr::without_provenance_mut(usize::from(enable))) }.map(|_r| ())
}
...
pub fn capability_is_in_bounding_set(capability: Capability) -> io::Result<bool> {
    unsafe { prctl_2args(PR_CAPBSET_READ, ptr::without_provenance_mut(capability as usize)) }.map(|r| r != 0)
}
...
pub fn remove_capability_from_bounding_set(capability: Capability) -> io::Result<()> {
    unsafe { prctl_2args(PR_CAPBSET_DROP, ptr::without_provenance_mut(capability as usize)) }.map(|_r| ())
}
...
pub fn set_capabilities_secure_bits(bits: CapabilitiesSecureBits) -> io::Result<()> {
    unsafe { prctl_2args(PR_SET_SECUREBITS, ptr::without_provenance_mut(bits.bits() as usize)) }.map(|_r| ())
}
...
pub fn capability_is_in_ambient_set(capability: Capability) -> io::Result<bool> {
    ...
    unsafe { prctl_3args(PR_CAP_AMBIENT, ptr::without_provenance_mut(PR_CAP_AMBIENT_IS_SET), cap) }
        .map(|r| r != 0)
}
...
pub fn clear_ambient_capability_set() -> io::Result<()> {
    unsafe { prctl_2args(PR_CAP_AMBIENT, ptr::without_provenance_mut(PR_CAP_AMBIENT_CLEAR_ALL)) }.map(|_r| ())
}
...
pub fn configure_capability_in_ambient_set(capability: Capability, enable: bool) -> io::Result<()> {
    ...
    unsafe { prctl_3args(PR_CAP_AMBIENT, ptr::without_provenance_mut(sub_operation), cap) }.map(|_r| ())
}
...
pub unsafe fn reset_pointer_authentication_keys(
    ...
    prctl_2args(PR_PAC_RESET_KEYS, ptr::without_provenance_mut(keys as usize)).map(|_r| ())
}
...
pub unsafe fn set_current_tagged_address_mode(
    ...
    prctl_2args(PR_SET_TAGGED_ADDR_CTRL, ptr::without_provenance_mut(config as usize)).map(|_r| ())
}
